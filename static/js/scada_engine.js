/* ============================================================
   SCADA ENGINE v3.0 — Packages Group Professional Theme
...
function updateDashboard(data) {
    ...
    for (const [id, d] of Object.entries(data)) {
        ...
        if (Math.abs(newV - prev.v) > 0.3) { vEl.classList.add('flash'); setTimeout(()=>vEl.classList.remove('flash'),600); }
        ...
        if (Math.abs(newKW - prev.kw) > 1) { kwEl.classList.add('flash'); setTimeout(()=>kwEl.classList.remove('flash'),600); }
